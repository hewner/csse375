    ...
    private Bet getAndParseBet() {
        while (true) {
            ...
            if (bet.contains("even"))
                return new Bet(Bet.BetOption.EVEN);

            if (bet.contains("odd"))
                return new Bet(Bet.BetOption.ODD);

            if (bet.contains("red"))
                return new Bet(Bet.BetOption.RED);

            if (bet.contains("black"))
                return new Bet(Bet.BetOption.BLACK);
            
            try {
                int numBet = Integer.parseInt(bet);
                if (numBet >= 1 && numBet < 34) {
                    return new Bet(numBet);
                }

...
public class Game
{
    ...
    private Bet getAndParseBet() {
        while (true) {
            ...
            if(OddEvenBet.isThisKindOfThing(bet))
                return OddEvenBet.parseBet(bet);
            
            if(RedBlackBet.isThisKindOfBet(bet))
                return RedBlackBet.parseBet(bet);
            
            if(ThreeNumbersBet.isThisKindOfBet(bet))
                return ThreeNumbersBet.parseBet(bet);
